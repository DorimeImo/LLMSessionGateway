        ...
        public static Result<T> Handle<T>(Exception ex, string source, string operation, IStructuredLogger logger)
        {
            switch (ex)
            {
                case OperationCanceledException:
                    logger.LogWarning(source, operation, "Token acquisition was canceled.");
                ...
                case CredentialUnavailableException cue:
                    logger.LogError(source, operation,
                        "No usable credentials found for token acquisition. Ensure Managed Identity / env credentials are configured.",
                        cue);
                ...
                case AuthenticationFailedException afe:
                    logger.LogError(source, operation,
                        $"Authentication failed while acquiring token. Message: {afe.Message}", afe);
                    return Result<T>.Failure("Authentication failed", errorCode: "AUTH_FAILED", isRetryable: true);

                case RequestFailedException rfe:
                    logger.LogError(source, operation,
                        $"Authority returned error. Status: {rfe.Status}, ErrorCode: {rfe.ErrorCode ?? "UNKNOWN"}, Message: {rfe.Message}",
                        rfe);
                    var retryable = rfe.Status is 408 or 429 or 500 or 502 or 503;
                    return Result<T>.Failure("Authority request failed", errorCode: rfe.ErrorCode ?? "AUTHORITY_ERROR", isRetryable: retryable);
